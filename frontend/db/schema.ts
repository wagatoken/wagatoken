...
export const wagaCoffeeBatches = pgTable('waga_coffee_batches', {
  ...
  quantity: integer().notNull(), // Current quantity
  price: decimal({ precision: 10, scale: 2 }).notNull(), // Price per unit in USD
  packaging: varchar({ length: 20 }).notNull(), // '250g' or '500g'
  ...
  roastProfile: varchar({ length: 100 }), // From IPFS properties
  roastDate: varchar({ length: 50 }), // From IPFS properties
